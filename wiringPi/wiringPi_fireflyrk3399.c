...
//===========================================
int rk3399_gpio_set_direction(int rk3399gpio, int input)
{
    volatile uint32_t *gpio;
    int pin = rk3399gpio % 32;
    uint32_t data;
    gpio = rk3399_gpioRegMap[rk3399gpio/32];

    data = *(gpio + GPIO_SWPORT_DDR);
    /* set bit to 1 for output, 0 for input */
    if (!input)
        data |= BIT(pin);
    else
        data &= ~BIT(pin);
    *(gpio + GPIO_SWPORT_DDR) = data;
}
